...
def clean_cosmetic_df(cosmetic_df: DataFrame) -> DataFrame:
    ...
    cosmetic_df = cosmetic_df.withColumnRenamed("product_id", "cosmetic_id") \
...
def clean_mapping_df(mapping_df: DataFrame) -> DataFrame:
    # Standardize column names
    mapping_df = mapping_df.withColumnRenamed("product_id_events", "cosmetic_id") \
                           .withColumnRenamed("product_id_reviews", "review_id")
...
def clean_reviews_df(reviews_df: DataFrame) -> DataFrame:
    # Standardize column names
    reviews_df = reviews_df.withColumnRenamed("product_id", "review_id") \

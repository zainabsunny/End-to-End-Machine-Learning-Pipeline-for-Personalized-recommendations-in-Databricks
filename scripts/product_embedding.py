...
def generate_topic_sentiment_embeddings(df):
    ...
    """
    df = df.withColumn("topic_embedding", minilm_embedding_udf(col("topics")))
    df = df.withColumn("sentiment_embedding", minilm_embedding_udf(col("sentiment")))
    return df
...
def merge_embeddings(df):
    ...
    df = df.withColumn("combined_embedding", coalesce(col("combined_embedding"), array().cast("array<float>")))
    df = df.withColumn("topic_embedding", coalesce(col("topic_embedding"), array().cast("array<float>")))
    df = df.withColumn("sentiment_embedding", coalesce(col("sentiment_embedding"), array().cast("array<float>")))
...
def clean_aggregated_embeddings(df, embedding_col="final_embedding", expected_dim=384):
    return df.withColumn(
        embedding_col,
        expr(f"IF(size({embedding_col}) = {expected_dim}, {embedding_col}, array({', '.join(['0.0'] * expected_dim)}))")
    )
...
def check_ncf_output(model, ncf_recs: dict, interaction_data: pd.DataFrame):
    """
    Perform basic quality checks on the NCF model output.
    
    :param model: Trained NCF model (PyTorch)
    :param ncf_recs: Dictionary of user-based recommendations
    :param interaction_data: Pandas DataFrame of user-item interactions
    """
    print("\n=== NCF Recommender Output Checks ===")

    # Check that model is not None
    if model is None:
        print("[CHECK] ❌ The NCF model is None!")
    else:
        print("[CHECK] ✅ NCF model is present.")

    # --- Check ncf_recs (Recommendation Dictionary) ---
    print("\nChecking NCF recommendations...")
    if not ncf_recs:
        print("[CHECK] ❌ NCF recommendations are empty!")
    else:
        print(f"[CHECK] ✅ NCF has recommendations for {len(ncf_recs)} users.")

        # Example: Checking structure of recommendations
        sample_user = next(iter(ncf_recs.keys()), None)
        if sample_user:
            if isinstance(ncf_recs[sample_user], np.ndarray) or isinstance(ncf_recs[sample_user], list):
                print("[CHECK] ✅ Recommendations are in the expected format (list of product IDs).")
            else:
                print("[CHECK] ❌ Recommendations format is incorrect!")

    # --- Check interaction_data (Training Data) ---
    print("\nChecking interaction_data DataFrame...")
    if interaction_data.empty:
        print("[CHECK] ❌ interaction_data DataFrame is empty!")
    else:
        print(f"[CHECK] ✅ interaction_data has {interaction_data.shape[0]} rows and {interaction_data.shape[1]} columns.")

        # Example: Check required columns
        expected_cols = {"user_id", "cosmetic_product_id", "interaction_score"}
        missing_cols = expected_cols - set(interaction_data.columns)
        if missing_cols:
            print(f"[CHECK] ❌ Missing columns in interaction_data: {missing_cols}")
        else:
            print("[CHECK] ✅ All expected columns in interaction_data are present.")

    print("\n=== NCF Recommender Output Checks Complete ===\n")
